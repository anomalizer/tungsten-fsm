...
import com.continuent.tungsten.fsm.core.test.SampleAction;
 ...
 * basic event dispatch, dispatch thread start and stop, and processing of
 * out-of-band events.
 * 
...
public class EventDispatcherTest extends TestCase
{
    ...
    public void testStartStop() throws Exception
    {
        ...
        Assert.assertTrue("should be running after start",
                dispatcher.isRunning());
        dispatcher.stop();
        Assert.assertFalse("Should not be running after stop",
                dispatcher.isRunning());
    }
    ...
    public void testEventSubmit() throws Exception
    {
        ...
        Assert.assertEquals("should be in start state", "START", sm.getState()
                .getName());
        ...
        Assert.assertEquals("should be in end state", "END", sm.getState()
                .getName());
    ...
    public void testTransitionRollback() throws Exception
    {
        ...
        StateMachine sm = this
                .createSimpleStateMachine(SampleAction.Outcome.ROLLBACK);
        EventDispatcherTask dispatcher = new EventDispatcherTask(sm);
        SampleStatusListener listener = new SampleStatusListener();
        dispatcher.setListener(listener);
    ...
    public void testMultiEventSubmit() throws Exception
    {
        ...
        for (int i = 0; i < 1000; i++)
        {
            ...
            Assert.assertEquals("Iteration [" + i + "] transition to MIDDLE",
                    "MIDDLE", sm.getState().getName());
            Assert.assertTrue("Iteration [" + i
                    + "] transition to MIDDLE (status)", s1.isSuccessful());
            ...
            Assert.assertEquals("Iteration [" + i + "] transition to START",
                    "START", sm.getState().getName());
            Assert.assertTrue("Iteration [" + i
                    + "] transition to START (status)", s2.isSuccessful());
        }
    ...
    public void testCancelEntryAction() throws Exception
    {
        ...
        try
        {
            ...
            throw new Exception("Entry action block failed: success="
                    + status.isSuccessful());
        }
        ...
        Assert.assertEquals("Should be in start state again", "START", sm
                .getState().getName());
    ...
    public void testCancelNonRunning() throws Exception
    {
        ...
        Future<EventStatus> future2 = dispatcher.put(e2);
        EventRequest er = dispatcher.put(e2);
        er.cancel(true);
        ...
        Assert.assertFalse("Pending event should have failed",
                status1.isSuccessful());
        Assert.assertTrue("Pending event should be cancelled",
                status1.isCancelled());
    }
    ...
    public StateMachine createSimpleStateMachine(SampleAction.Outcome outcome)
            throws Exception
    {
        ...
        Transition s_2_e = new Transition(new RegexGuard("END"), start, null,
                end);
        SampleAction action = new SampleAction();
        if (outcome != null)
            action.setOutcome(outcome);
        Transition s_2_s = new Transition(new RegexGuard("START"), start,
                action, start);
    ...
    public StateMachine createTimedStateMachine(int middleEntrySecs,
            int transitionToMiddleSecs, int middleExitSecs) throws Exception
    {
        ...
        State middle = new State("MIDDLE", StateType.END, null,
                middleEntryAction, middleEntryAction);

        // Following transitions are immediate.
        Transition m_2_s = new Transition(new RegexGuard("START"), middle,
                null, start);
        Transition s_2_e = new Transition(new RegexGuard("END"), start, null,
                end);
        ...
        Transition s_2_m = new Transition(new RegexGuard("MIDDLE"), start,
                transitionAction, middle);
